...
def sample_from_model(coefficients, generator1, cond1, contrast, n_time, x_init, T, opt):
    x = x_init
    ...
    with torch.no_grad():
        for i in reversed(range(n_time)):
            ...
            x_0_1 = generator1(x, cond1, contrast, t_time, latent_z)
...
def load_checkpoint_auto(netG, device='cuda:0'):
...
def euclidean_region_contrast(hf_embed_actual, hf_embed_synth):
    """
    hf_embed_actual, hf_embed_synth: [B, R, C] - region-level features
    Euclidean contrastive loss (InfoNCE-style) across anatomical regions
    """
    B, R, C = hf_embed_actual.shape
    loss = 0

    for b in range(B):
        actual = hf_embed_actual[b]  # [R, C]
        synth = hf_embed_synth[b]  # [R, C]

        for i in range(R):
            # Positive pair: same region
            pos = torch.norm(actual[i] - synth[i], p=2)

            # Negative pairs: different regions
            neg_dists = torch.stack([
                torch.norm(actual[i] - synth[j], p=2)
                for j in range(R) if j != i
            ])

            # Contrastive logits and labels
            logits = -torch.cat([pos.unsqueeze(0), neg_dists], dim=0)  # More similar = higher logit
...
def extract_region_embeddings(feat_map, seg_mask, original_size=(256, 256)):
    B, _, H, W = seg_mask.shape
    upsampled_feat_map = F.interpolate(feat_map, size=original_size, mode='bilinear', align_corners=False)
    classes = [61, 62, 63, 64, 65, 66, 67, 68, 69, 70, 71, 72, 73, 74]

    region_embeddings = []
    for r in classes:
        mask = (seg_mask == r).float()  # [B, 1, H, W]
        pooled = (upsampled_feat_map * mask).sum(dim=[2, 3])  # [B, C]
        area = mask.sum(dim=[2, 3]) + 1e-5
        pooled = pooled / area
        region_embeddings.append(pooled)  # [B, C]

    return torch.stack(region_embeddings, dim=1)  # Final shape: [B, R, C]


def train_syndiff(rank, gpu, args):
    from backbones.diff_disc import Discriminator_large

    from backbones.diff_gen import MultiFieldDiff
    from backbones.autoen import AutoEncoder
    ...
    dataset = MixedFieldDataset(train=True, input_path=args.input_path)
    dataset_val = MixedFieldDataset(train=False, input_path=args.input_path)
    ...
    gen_diffusive_1 = MultiFieldDiff(args).to(device)

    gen_auto = AutoEncoder(args).to(device)
    load_checkpoint_auto(gen_auto, device)
    ...
    if args.resume:
        checkpoint_file = os.path.join(exp_path, 'contentp.pth')
        checkpoint = torch.load(checkpoint_file, map_location=device)
    ...
    for epoch in range(init_epoch, args.num_epoch + 1):
        ...
        for iteration, (lf_img, hf_img, lf_hf_seg, task_id) in enumerate(data_loader):
            cond_data = lf_img.to(device, non_blocking=True)  # condition (ULF or HF)
            real_data = hf_img.to(device, non_blocking=True)  # target    (HF or UHF)
            task_seg = lf_hf_seg.to(device, non_blocking=True)
            ...
            mask0 = (task_id == 0)  # ULF->HF
            mask1 = (task_id == 1)  # HF->UHF

            if mask0.any():
                lf_data = cond_data[mask0]
                hf_data = real_data[mask0]
                seg_mask = task_seg[mask0]

            if mask1.any():
                lf_data = cond_data[mask1]
                hf_data = real_data[mask1]
                seg_mask = task_seg[mask1]
            ...
            t2 = torch.randint(0, args.num_timesteps, (hf_data.size(0),), device=device)

            x2_t, x2_tp1 = q_sample_pairs(coeff, hf_data, t2)
            x2_t.requires_grad = True

            # train discriminator with real
            D2_real = disc_diffusive_2(x2_t, t2, x2_tp1.detach())
            ...
            _, hf_feat = gen_auto(hf_data)

            x2_0_predict_diff_g1 = gen_diffusive_1(x2_tp1.detach(), lf_data, contrast, t2, latent_z2)

            x2_pos_sample_g1 = sample_posterior(pos_coeff, x2_0_predict_diff_g1[:, [0], :], x2_tp1, t2)

            # D output for fake sample x_pos_sample

            output2_g1 = disc_diffusive_2(x2_pos_sample_g1, t2, x2_tp1.detach())

            errD2_fake2_g1 = (F.softplus(output2_g1)).mean()

            errD_fake2 = errD2_fake2_g1
            errD_fake2.backward()
            ...
            # D output for fake sample x_pos_sample
            output2_g1 = disc_diffusive_2(x2_pos_sample_g1.detach(), t2, x2_tp1.detach())

            _, hf_synth_feat = gen_auto(x2_0_predict_diff_g1[:, [0], :])

            actual_regions = extract_region_embeddings(hf_feat, seg_mask, original_size=(256, 256))
            hf_regions_synth = extract_region_embeddings(hf_synth_feat, seg_mask, original_size=(256, 256))

            feat_loss = euclidean_region_contrast(actual_regions, hf_regions_synth)
            ...
            errG1_2_L1 = F.l1_loss(x2_0_predict_diff_g1[:, [0], :], hf_data)

            errG_L1 = errG1_2_L1

            errG = errG_adv + (args.lambda_l1_loss * errG_L1) + (0.3 * feat_loss)
            errG.backward()
        ...
        if rank == 0:
            ...
            x2_t = torch.randn_like(hf_data)

            fake_sample1 = sample_from_model(pos_coeff, gen_diffusive_1, lf_data, contrast,
                                             args.num_timesteps, x2_t, T, args)

            fake_sample1 = torch.cat((lf_data, hf_data, fake_sample1), axis=-1)
        ...
        for iteration, (lf_img_val, hf_img_val, lf_hf_seg_val, task_id_val) in enumerate(data_loader):
            cond_data = lf_img_val.to(device, non_blocking=True)  # condition (ULF or HF)
            real_data = hf_img_val.to(device, non_blocking=True)  # target    (HF or UHF)
            task_id = task_id_val.to(device, non_blocking=True)
            ...
            real_data = hf_data.to(device, non_blocking=True)
            source_data = lf_data.to(device, non_blocking=True)

            x1_t = torch.randn_like(real_data)

            # diffusion steps
            fake_sample = sample_from_model(pos_coeff, gen_diffusive_1, source_data, contrast,
            ...
            val_l1_loss[0, epoch, iteration] = abs(fake_sample - real_data).mean()

        print(np.nanmean(val_l1_loss[0, epoch, :]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input_path', default='input_dir_path')
    parser.add_argument('--output_path', default='output_dir_path')
