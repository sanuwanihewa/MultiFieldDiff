...
def sample_and_test(args):
    ...
    gen_diffusive_2 = MultiFieldDiff(args).to(device)
    ...
    print('loaded')
    load_checkpoint(checkpoint_file, gen_diffusive_2, 'gen_diffusive_1', epoch=str(epoch_chosen), device=device)

    dataset = MixedFieldDataset(train=False, input_path=args.input_path)  # update test path in dataset.py
    ...
    for iteration, (lf_img, hf_img, segs, task_id) in enumerate(data_loader):
        ...
        if (task_id == 1).all():
            contrast = 'UHF'
            print('processing UHF')

        mask0 = (task_id == 0)  # ULF->HF
        mask1 = (task_id == 1)  # HF->UHF

        if mask0.any():
            cond_data = cond_data[mask0]
            real_data = real_data[mask0]

        if mask1.any():
            cond_data = cond_data[mask1]
            real_data = real_data[mask1]

        x1_t = torch.randn_like(cond_data)

        fake_sample = sample_from_model(pos_coeff, gen_diffusive_2, cond_data, contrast,
                                        args.num_timesteps, x1_t, T, args)

        fake_sample = to_range_0_1(fake_sample);
        fake_sample2_p = fake_sample / fake_sample.mean()
        real_data = to_range_0_1(real_data);
        real_data_p = real_data / real_data.mean()
        cond_data = to_range_0_1(cond_data);
        cond_data_p = cond_data / cond_data.mean()

        test_results_all = torch.cat((cond_data_p.cuda(), fake_sample2_p.cuda(), real_data_p.cuda()), axis=-1)

        torchvision.utils.save_image(test_results_all, '{}/{}_samples_{}.jpg'.format(save_dir, phase, iteration),
                                     normalize=True)
