...
import torch
import numpy as np
from .field_adapter import AdaptiveFieldGen
...
class MultiFieldDiff(nn.Module):
    """diffusion model"""
    ...
    def __init__(self, config):
        ...
        modules.append(ConvBlock(in_ch=channels, out_ch=nf))
        modules.append(ConvBlock(in_ch=nf * 2, out_ch=nf))
        ...
        self.adapter1 = AdaptiveFieldGen(['HF', 'UHF'], 64, 256)
        self.adapter2 = AdaptiveFieldGen(['HF', 'UHF'], 256, 512)

    def forward(self, x, cond, contrast, time_cond, z):
        # timestep/noise_level embedding; only for continuous training
        ...
        all_feat = modules[m_idx](torch.cat((x_feat, cond_feat), axis=1))
        m_idx += 1

        all_feat = self.adapter1(all_feat, contrast)
