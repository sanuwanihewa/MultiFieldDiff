...
class ResnetBlockBigGANpp_Adagn(nn.Module):
    def __init__(self, act, in_ch, out_ch=None, temb_dim=None, zemb_dim=None, up=False, down=False,
        ...
        self.Conv_f = conv3x3(out_ch * 2, out_ch)
    ...
    def forward(self, x, temb=None, zemb=None):
        ...
        if temb is not None:
            scale_msa = self.Dense_0(self.act(temb))[:, :, None, None]

            x_patch_fft = torch.fft.rfft2(h.float())
            x_patch_fft = x_patch_fft * scale_msa
            h_freq_mod = torch.fft.irfft2(x_patch_fft)

            h_spatial_mod = h + scale_msa

            h_fused = torch.cat([h_freq_mod, h_spatial_mod], dim=1)  # Shape: [B, 2C, H, W]
            h = self.Conv_f(h_fused)  # Back to shape [B, C, H, W]
...
class ResnetBlockBigGANpp_Adagn_autoen(nn.Module):
    def __init__(self, act, in_ch, out_ch=None, temb_dim=None, zemb_dim=None, up=False, down=False,
                 dropout=0.1, fir=False, fir_kernel=(1, 3, 3, 1),
                 skip_rescale=True, init_scale=0.):
        super().__init__()
